		...
		<div id="main_content">	
			<?php
				...
				if(isset($_GET['delete_post'])){
					...
					$post_query = mysqli_query($GLOBALS['mysqli'], "SELECT * FROM post WHERE id_post=$id_post");
					$post = mysqli_fetch_assoc($post_query);
					$thread_id = $post['id_thread'];
					$current_thread_query = mysqli_query($GLOBALS['mysqli'], "SELECT * FROM thread WHERE id_thread='$thread_id'");
					...
					$user_query = mysqli_query($GLOBALS['mysqli'], "SELECT id_user FROM user WHERE user_name='$current_user'");
					$user = mysqli_fetch_assoc($user_query);
					
					if($permit_level > 7){
						mysqli_query($GLOBALS['mysqli'], "DELETE FROM attachment WHERE id_post=$id_post");
						mysqli_query($GLOBALS['mysqli'], "UPDATE post SET post_content='<b><i>Post deleted by Administration</b></i>' WHERE id_post=$id_post");
						...
						mysqli_query($mysqli, "UPDATE post SET post_parameters='$post_param_json' WHERE id_post=$id_post");
						...
						if($post['id_user'] == $user['id_user']){
							mysqli_query($GLOBALS['mysqli'], "DELETE FROM attachment WHERE id_post=$id_post");
							mysqli_query($GLOBALS['mysqli'], "UPDATE post SET post_content='<i>Post deleted by user</i>' WHERE id_post=$id_post");	
